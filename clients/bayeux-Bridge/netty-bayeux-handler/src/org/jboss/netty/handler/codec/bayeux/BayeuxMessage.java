...
public class BayeuxMessage implements BayeuxInterface {
    ...
    BayeuxConnection.TYPE[] supportedConnectionTypes;//A array, like ["long-polling","http-streaming","flash","iframe"]
    ...
    BayeuxConnection.TYPE connectionType;// Connection type, must be one of four supported connection types
    ...
    public String toJSON() {
        ...
        if (supportedConnectionTypes != null && supportedConnectionTypes.length != 0) {
            ...
            for (int i = 0; i < supportedConnectionTypes.length; i++) {
                supportedConnectionTypeList.add(BayeuxConnection.getValueOfType(supportedConnectionTypes[i]));
            }
            json.append(",\"supportedConnectionTypes\":").append(JSONParser.toJSON(supportedConnectionTypeList));
        }
        ...
        if (connectionType != null) {
            json.append(",\"connectionType\":").append(JSONParser.toJSON(BayeuxConnection.getValueOfType(connectionType)));
        }
    ...
    public boolean equals(Object o) {
        ...
        if (!isEqual(this.supportedConnectionTypes, bayeux.supportedConnectionTypes)) {
            return false;
        }
