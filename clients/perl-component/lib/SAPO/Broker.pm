...
sub poll {
    ...
    my $queue_group = $args{queue_group}
        || '';    # if not passed and msg_type is TOPIC_AS_QUEUE, this will fallback to the hostname
...
sub receive {
    ...
    my $event = _parse_soap_message( $buf, 'http://services.sapo.pt/broker',
        'BrokerMessage', qw(TextPayload MessageId DestinationName Timestamp Expiration) );

    return $self->{retstruct}
        || ( $self->{msg_type} && $self->{msg_type} eq 'TOPIC_AS_QUEUE' ) ? $event : $event->{TextPayload};
} ## end sub receive
...
sub ack {
    ...
    my $msg = "<soapenv:Envelope xmlns:soapenv='http://www.w3.org/2003/05/soap-envelope'>
		<soapenv:Body>
...
sub _open_sock {
    ...
    if ( $proto eq 'tcp' ) {
        $params{Reuse} = 1, $params{MultiHomed} = 1, $params{Blocking} = 1, $params{Type} = SOCK_STREAM,;
    }
    ...
    if ( $self->{hosts} ) {
        for my $peer ( @{ $self->{hosts} } ) {
            ...
            $sock = $self->_open_sock(
                $peer->{host},
                $peer->{port} || $self->{port},
                $peer->{timeout} || $self->{timeout}, $proto
            );
        ...
        $sock = $self->_open_sock( $self->{host}, $self->{port}, $self->{timeout}, $self->{proto} );
...
sub _parse_with_libxml {
    ...
    my $xpath;
    eval { $xpath = XML::LibXML::XPathContext->new( $libxml_parser->parse_string($xml) ); };
