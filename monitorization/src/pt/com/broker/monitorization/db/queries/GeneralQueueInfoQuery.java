...
public class GeneralQueueInfoQuery
{
	...
	private static String QUERY = "SELECT  agents.agent_name , last_event_for_subject_predicate_agent(?, 'queue-size', agents.agent_name, now(), '00:05') AS queuesize , last_event_for_subject_predicate_agent(?, 'input-rate', agents.agent_name, now(), '00:05') AS inputrate  , last_event_for_subject_predicate_agent(?, 'output-rate', agents.agent_name, now(), '00:05') AS outputrate , last_event_for_subject_predicate_agent(?, 'failed-rate', agents.agent_name, now(), '00:05') AS failed , last_event_for_subject_predicate_agent(?, 'expired-rate', agents.agent_name, now(), '00:05') AS expired , last_event_for_subject_predicate_agent(?, 'redelivered-rate', agents.agent_name, now(), '00:05') AS redelivered , last_event_for_subject_predicate_agent(?, 'subscriptions', agents.agent_name, now(), '00:05') AS subscriptions FROM (SELECT DISTINCT agent_name FROM raw_data WHERE event_time > (now() - '00:05'::time) AND subject=?) AS agents Order BY 2 DESC";
