...
{
	private static final Charset UTF_8_CS = Charset.forName("utf-8");
	...
	private long _expiration = _timestamp + DEFAULT_EXPIRY;
	...
	{		
		...
		int lcon = oin.readInt();
		byte[] bcontent = new byte[lcon];	
		oin.readFully(bcontent);
		_content = new String(bcontent, UTF_8_CS);
	}
	...
	public void writeExternal(ObjectOutput oout) throws IOException
	{
		...
		byte[] bcontent = _content.getBytes(UTF_8_CS);		
		oout.writeInt(bcontent.length);		
		oout.write(bcontent);
	}
