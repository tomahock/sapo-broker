    ...
    /*private static final void brokerAgentsLaucher(){
    	if(lauchAgent){
    		for(int i = 0; i < nAgents; i++){
    			String agentConfigPath = System.getProperty(String.format("%s-%s", Start.AGENT_CONFIG_PATH_PROPERTY, i));
    			String globalConfigPath = System.getProperty(String.format("%s-%s", Start.GLOBAL_CONFIG_PATH_PROPERTY, i));
    			log.debug("Launching agent with configuration files: {}; {}", agentConfigPath, globalConfigPath);
    			System.setProperty(Start.AGENT_CONFIG_PATH_PROPERTY, agentConfigPath);
    			System.setProperty(Start.GLOBAL_CONFIG_PATH_PROPERTY, globalConfigPath);
    			log.debug("Agent config path in system property: {}", System.getProperty(Start.AGENT_CONFIG_PATH_PROPERTY));
    			log.debug("Agent global config path in system property: {}", System.getProperty(Start.GLOBAL_CONFIG_PATH_PROPERTY));
    			try {
					int retVal = JavaProcess.exec(Start.class);
					log.debug("Returned value for launching Agent: {}", retVal);
